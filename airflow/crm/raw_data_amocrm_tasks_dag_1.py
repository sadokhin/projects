"""
load data about tasks from tasks in amocrm and push this data into google bigquery
"""
...
with models.DAG(
    ...
    async def task_list():
        ...
        response = _call_amo_method('tasks', data)
        data_resp = response.json()
        leads = data_resp['_embedded']['tasks']
        while 'next' in data_resp.get('_links') and iteration < MAX_RUN_ITERATIONS:
            ...
            data_resp = response.json()
            leads.extend(data_resp['_embedded']['tasks'])
            iteration += 1
        return leads

    def load_tasks():
        tasks = asyncio.run(task_list())
        print(len(tasks))

        df = pandas.json_normalize(tasks)
