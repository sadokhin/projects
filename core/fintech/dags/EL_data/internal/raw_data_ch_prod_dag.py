...
DAG_NAME = 'raw_data_ch_prod'
DAG_NUMBER = '1'
DESCRIPTION = 'Extract and load data from Clickhouse Prod to Analytics DWH'
...
with models.DAG(
        ...
        schedule_interval='10 0-23/1 * * *',
        tags = ['ch','prod','extract','load', 'sql'],
) as dag:
    ...
    def read_exported_tables():
        ...
        prod_config = next((entry for entry in data['clickhouse'] if entry['instance'] == 'ch_prod'), None)
    ...
    def main_func(database,table_config):
        ...
        # Add mark if table from taffy databases as of they have same naming
        taffy_mark = ''
        if 'taffy' in database:
            taffy_mark = 'taffy_'

        # Get current max cursor value from analytics table
        cursor_value = int_client_ch_analytics.query_to_df(
            f'select max({cursor_field}) from raw_ch_prod.{taffy_mark}{table_name}'
        ).iloc[0, 0]
        ...
        df = int_client_ch_analytics.optimize_data_types_to_ch_format(
            table_name=f'raw_ch_prod.{taffy_mark}{table_name}',
            dataframe=df
        )
        ...
        for i in range(len(chunks)):    
            int_client_ch_analytics.insert_df(f'raw_ch_prod.{taffy_mark}{table_name}', chunks[i])
    ...
    for db_config in prod_config:
        ...
        run_export = [PythonOperator(
            task_id='exporting_{}{}'.format('taffy_' if 'taffy' in database else '', table_config['name']),
